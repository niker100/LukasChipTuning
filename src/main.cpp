...
void adjustTimeWithDrift() {
  if (numDeltas == 0) return; // No deltas to calculate drift

  double meanDeltaPerSecond = 0;
  for (Delta d : deltas) {
    meanDeltaPerSecond += d.delta_seconds / d.duration;
  }

  meanDeltaPerSecond /= numDeltas;
  int meanDeltaPerWeek = meanDeltaPerSecond * 60 * 60 * 24 * 7;

  // Adjust internal time with mean drift per week
  time_t internalTime;
  time(&internalTime);
  internalTime += meanDeltaPerWeek;  
  struct timeval tv = { .tv_sec = internalTime };
...
void displayDeltas() {
  ...
  display.println("Deltas:");
  for (int i = 0; i < numDeltas; i++) {
    display.setCursor((i % 4) * 64, 16 + (i / 4) * 8);
    display.print("Delta ");
    display.print(i);
    display.print(": ");
    display.println(deltas[i].delta_seconds);
  }
  display.display();
}
*/
...
void loop() {
  ...
  */
  ...
  if (difftime(now, lastSetTimestamp) > 60 * 60 * 24 * 7) { // Adjust once a week
    adjustTimeWithDrift();
  }
